...
## Administration gaps closed — 16 September 2026

- PATCH /api/plans/{plan}: administrator, non-own plan; required key_gaps and
  weekly_focus, max 2000. Parent row is locked; only draft state is accepted.
  Status/member changes through this endpoint are prohibited.
- GET /api/members: administrator only; name search and pagination, 20 per page.
  Payload contains member id/name/email and plan link metadata, never password.
- PATCH /api/members/{member}: administrator only, member targets only; name and
  unique sign-in email. Role/password mutation prohibited; changed email resets
  email_verified_at. Account deletion and role elevation are intentionally absent.
- Vue /skills and /members remove the catalogue/directory Postman dependency.
  Skills reuse the existing policy and open-week retirement guard. No schema change.

## Repeat development cycles — 17 September 2026

- Completed plans remain immutable and are retained as history. A member may start
  another plan only when every earlier cycle is completed.
- `development_plans.cycle_number` is assigned by the server under a member-row
  lock. The `(user_id, cycle_number)` pair is unique; clients cannot submit their
  own cycle number.
- A member can have at most one draft or active plan through the progression
  service. The eligible-members endpoint includes new members and members whose
  latest cycle is completed, and reports the number of the next cycle in the UI.
- Plan lists expose every historical cycle. Account and member summaries resolve
  the highest numbered cycle as the latest one.
- Verified with 51 PHP tests (316 assertions), 31 Vue component tests, 9 frontend
  authentication tests, a production build, and the live create-plan screen.
