...
it('creates a draft with the selected baseline and redirects once', async () => {
    let resolve;
    const request = vi.fn(path => path.startsWith('/members') ? Promise.resolve({ data: [{ id: 2, name: 'Member', development_plans_max_cycle_number: 1 }] }) : path.startsWith('/skills') ? Promise.resolve({ data: [skill] }) : new Promise(r => resolve = r));
    const w = render(CreatePlan, request); await flushPromises();
    expect(w.text()).toContain('Start cycle 2');
