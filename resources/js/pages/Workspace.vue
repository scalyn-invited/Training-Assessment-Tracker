...
<WorkspaceShell>
                ...
                <div class="overview-grid">
                    <section class="white-card"><div class="card-heading"><h2>Your account</h2><button class="text-button" @click="refresh" :disabled="loading">{{ loading ? 'Checking…' : 'Refresh ↻' }}</button></div>
                        ...
                        <dl v-else-if="profile" class="profile-list"><div><dt>Name</dt><dd>{{ profile.name }}</dd></div><div><dt>Email</dt><dd>{{ profile.email }}</dd></div><div><dt>Role</dt><dd><span class="role-tag">{{ profile.role }}</span></dd></div><div><dt>Latest development cycle</dt><dd>{{ profile.development_plan_status ? 'Cycle ' + profile.development_plan_cycle + ' · ' + profile.development_plan_status : 'Not assigned yet' }}</dd></div></dl>
                    </section>
