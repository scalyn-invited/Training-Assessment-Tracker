...
import DraftPlanEditor from '../components/DraftPlanEditor.vue';
...
<template>
    <WorkspaceShell>
        ...
        <ScreenState :loading="loading" :error="error" :empty="!plan" title="No plan available" description="Choose a plan from the directory to review its progress." @retry="load">
            <template v-if="plan">
                <div class="page-heading"><div><span class="section-number">CYCLE {{ plan.cycle_number }} · PLAN #{{ plan.id }}</span><h1>{{ plan.member?.name }}<span class="green">.</span></h1><p class="muted">{{ plan.weekly_focus }}</p></div><span class="status-tag" :class="plan.status">{{ plan.status }}</span></div>
                ...
                <DraftPlanEditor v-if="plan.status === 'draft' && state.user?.role === 'administrator' && state.user.id !== plan.user_id" :key="plan.id" :plan="plan" @saved="Object.assign(plan, $event)" />
