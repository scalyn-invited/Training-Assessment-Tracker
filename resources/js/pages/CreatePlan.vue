...
<ScreenState :loading="members.loading.value" :error="members.error.value" :empty="!members.data.value?.data?.length" title="No eligible members" description="A member becomes eligible after registration or after completing their current cycle." @retry="members.load">
<label for="plan-member">Member</label><select id="plan-member" v-model="form.user_id"><option disabled value="">Select member</option><option v-for="member in members.data.value?.data" :key="member.id" :value="member.id">{{ member.name }} · {{ member.development_plans_max_cycle_number ? 'Start cycle ' + (Number(member.development_plans_max_cycle_number) + 1) : 'First cycle' }}</option></select>
</ScreenState>
