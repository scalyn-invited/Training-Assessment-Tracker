    ...
    <WorkspaceShell>
        ...
        <section class="white-card screen-card">
            ...
            <ScreenState :loading="loading" :error="error" :empty="!data?.data?.length" title="No plans found" description="Try another status, or ask your administrator to assign a plan." @retry="load">
                <div class="table-scroll"><table><thead><tr><th>Member</th><th>Status</th><th>Skills</th><th>Current week</th><th><span class="sr-only">Action</span></th></tr></thead><tbody><tr v-for="plan in data?.data" :key="plan.id"><td><strong>{{ plan.member?.name }}</strong><small>Cycle {{ plan.cycle_number }} · Plan #{{ plan.id }}</small></td><td><span class="status-tag" :class="plan.status">{{ plan.status }}</span></td><td>{{ plan.skills_count }}</td><td>{{ plan.current_week ? 'Week ' + plan.current_week : plan.weekly_entries_count ? 'All weeks closed' : 'Not started' }}</td><td><RouterLink :to="'/plans/' + plan.id" class="table-link" :aria-label="'Open plan for ' + plan.member?.name + ', cycle ' + plan.cycle_number">View plan ↗</RouterLink></td></tr></tbody></table></div>
            </ScreenState>
