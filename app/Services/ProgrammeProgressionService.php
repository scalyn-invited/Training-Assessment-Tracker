...
{
    public function updateDraft(DevelopmentPlan $plan, User $actor, array $data): DevelopmentPlan
    {
        return DB::transaction(function () use ($plan, $actor, $data) {
            ...
            $plan->update(collect($data)->only(['key_gaps', 'weekly_focus'])->all());
    ...
    public function createPlan(User $actor, array $data): DevelopmentPlan
    {
        ...
        return DB::transaction(function () use ($actor, $data) {
            ...
            if ($member->developmentPlans()->where('status', '!=', PlanStatus::Completed)->exists()) {
                throw new ProgressionConflict('Complete the member’s current plan before starting another cycle.');
            }
            $cycleNumber = ((int) $member->developmentPlans()->max('cycle_number')) + 1;
            $plan = DevelopmentPlan::create([
                'user_id' => $member->id, 'cycle_number' => $cycleNumber, 'key_gaps' => $data['key_gaps'],
