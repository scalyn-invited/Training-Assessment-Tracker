...
{
    public function update(Request $request, DevelopmentPlan $plan, ProgrammeProgressionService $service): DevelopmentPlanResource
    {
        Gate::authorize('update', $plan);
        $data = $request->validate([
            'key_gaps' => ['required', 'string', 'max:2000'],
            'weekly_focus' => ['required', 'string', 'max:2000'],
            'status' => ['prohibited'], 'user_id' => ['prohibited'],
        ]);

        return new DevelopmentPlanResource($service->updateDraft($plan, $request->user(), $data));
    }
    ...
    public function eligibleMembers(Request $request): JsonResponse
    {
        ...
        $members = User::query()->select(['id', 'name'])->where('role', 'member')
            ->whereDoesntHave('developmentPlans', fn ($plans) => $plans->where('status', '!=', PlanStatus::Completed))
            ->withMax('developmentPlans', 'cycle_number')
